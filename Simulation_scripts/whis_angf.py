...
# This split is set so that it happens before the dce split. Different from other models but necessary so that mys pops have a time of coexcistence with the dce population for the test migration events
lower, upper = spl_dce, spl_anc # roughly 1 mio to 15 mio years ago (Generation time 7)
mu, sigma = spl_anc/2, spl_anc/5
X = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
...
###Migration start and end times MC/ME DCE (Test migration event) ###
# Migration start time
lower, upper = spl_dce, min(spl_mys,spl_dav) # from split of european and caucasiand davdii (Generation time 7)
mu, sigma = (min(spl_mys,spl_dav) - (min(spl_mys,spl_dav) - spl_dce)/2), (min(spl_mys,spl_dav) - (min(spl_mys,spl_dav) - spl_dce)/2) / 5
X = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
sm = np.round(X.rvs(2)) # 2 time
#plt.hist(sm * 7)
#max(sm * 7)
#min(sm * 7)
...
# Migration end time
lower, upper = spl_dce, ms_mcdce # from migration start (Generation time 7)
mu, sigma = (ms_mcdce - (ms_mcdce - spl_dce)/2), (ms_mcdce - (ms_mcdce - spl_dce)/2) / 10
X = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
em = np.round(X.rvs(1)) # 1 time
#plt.hist(em * 7)
#max(em * 7)
...
# Migration end time
lower, upper = spl_dce, ms_medce # from migration start (Generation time 7)
mu, sigma = (ms_medce - (ms_medce - spl_dce)/2), (ms_medce - (ms_medce - spl_dce)/2) / 10
X = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
em = np.round(X.rvs(1)) # 1 time
#plt.hist(em * 7)
#max(em * 7)
...
### Add test migration events ###
# Between mc and dce
# Set migration rates.
dem.set_migration_rate(source="mc", dest="dce", rate=0)
dem.set_migration_rate(source="dce", dest="mc", rate=0)

# Start of migration
dem.add_migration_rate_change(time=ms_mcdce, rate=mr_mcdce, source="mc", dest="dce")
dem.add_migration_rate_change(time=ms_mcdce, rate=mr_dcemc, source="dce", dest="mc")

# End of migration
dem.add_migration_rate_change(time=me_mcdce, rate=0, source="mc", dest="dce")
dem.add_migration_rate_change(time=me_mcdce, rate=0, source="dce", dest="mc")

# Between me and dce
# Set migration rates.
dem.set_migration_rate(source="me", dest="dce", rate=0)
dem.set_migration_rate(source="dce", dest="me", rate=0) 

# Start of migration
dem.add_migration_rate_change(time=ms_medce, rate=mr_medce, source="me", dest="dce")
dem.add_migration_rate_change(time=ms_medce, rate=mr_dceme, source="dce", dest="me")

# End of migration
dem.add_migration_rate_change(time=me_medce, rate=0, source="me", dest="dce")
dem.add_migration_rate_change(time=me_medce, rate=0, source="dce", dest="me")
