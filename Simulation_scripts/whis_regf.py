...
# Migration start time
lower, upper = 0, min(spl_mys,spl_dce) # from split of european and caucasiand davdii (Generation time 7)
mu, sigma = min(spl_mys,spl_dce) / 2, min(spl_mys,spl_dce) / 3
X = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
...
# Migration end time
lower, upper = 0, ms_mcdc # from migration start (Generation time 7)
mu, sigma = ms_mcdc / 2, ms_mcdc / 3
X = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
em = np.round(X.rvs(1)) # 1 time
#plt.hist(em * 7)
#max(em * 7)
...
# Migration end time
lower, upper = 0, ms_mede # from migration start (Generation time 7)
mu, sigma = ms_mede / 2, ms_mede / 3
X = stats.truncnorm((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma)
em = np.round(X.rvs(1)) # 1 time
#plt.hist(em * 7)
#max(em * 7)
...
# Between mc and dc
# Set migration rates.
dem.set_migration_rate(source="mc", dest="dc", rate=0)
dem.set_migration_rate(source="dc", dest="mc", rate=0)

# Start of migration
dem.add_migration_rate_change(time=ms_mcdc, rate=mr_mcdc, source="mc", dest="dc")
dem.add_migration_rate_change(time=ms_mcdc, rate=mr_dcmc, source="dc", dest="mc")

# End of migration
dem.add_migration_rate_change(time=me_mcdc, rate=0, source="mc", dest="dc")
dem.add_migration_rate_change(time=me_mcdc, rate=0, source="dc", dest="mc")

# Between me and de
# Set migration rates.
dem.set_migration_rate(source="me", dest="de", rate=0)
dem.set_migration_rate(source="de", dest="me", rate=0)

# Start of migration
dem.add_migration_rate_change(time=ms_mede, rate=mr_mede, source="me", dest="de")
dem.add_migration_rate_change(time=ms_mede, rate=mr_mede, source="de", dest="me")

# End of migration
dem.add_migration_rate_change(time=me_mede, rate=0, source="me", dest="de")
dem.add_migration_rate_change(time=me_mede, rate=0, source="de", dest="me")
